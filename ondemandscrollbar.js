    ...
    (function(member) { member.apply(Scrollbars.prototype); })(function() {
        ...
        this.attachEvents = function() {
            ...
            if (Browser.Opera && !this.scrollable.isRoot()) {
                // if ancestor's overflow style changed, this container's scroll will be reset(0,0).
            ...
            var delayAttach = runnable.call(this, function() {
                if (detach) {
            ...
            if (window.top !== window.self) {
                if (this.scrollable.isRoot()) {
                    this.focused = false;
                    detach.push(subscribe(window, 'focus', bind(this, function() { this.focused = true; })));
                    detach.push(subscribe(window, 'blur', bind(this, function() { this.focused = false; })));
                }
            }
            this.detachEvents = function() { if (detach) run(detach); detach = null; };
        };
        ...
        this.ev_view_wheel = function(e) {
            ...
            if (on_sb || target == this.container) {
                ...
                if (!do_scroll && axis == 2) {
                    ...
                    if (this.focused && !this.scrollable.isScrollable(2) && this.scrollable.isScrollable(1, u1d2)) {
                        if (this.scrollable.isRoot() || this.container.offsetHeight > window.innerHeight) {
                            axis = 1;
                            do_scroll = true;
                        }
                    }
                ...
                if (do_scroll && axis == 2 && Browser.Opera && this.scrollable.isRoot() && !this.focused) {
                    do_scroll = false;
                }
                else // :-P
                if (do_scroll || (on_sb && this.scrollable.isScrollable(axis)) || (this.focused && this.scrollable.isScrollable())) {
                    //TODO: needs focus controll?
                    e.preventDefault();
...
// ==UserScript==
// @name      ondemand scrollbar plugin - horizontal pre
// @version   1.00
// @include   *
// ==/UserScript==
(function() {
    ...
    function init() {
        with (Namespace('http://www.hatena.ne.jp/miya2000/scrollbar/', opera || window)) {
            var pn = document.getElementsByTagName('pre');
            for (var i = 0, len = pn.length; i < len; i++) {
                var pre = pn[i];
                if (Scrollable.fn.isScrollableNode(pre, 1) && Scrollable.fn.isScrollable(pre, 1)) {
                    var scr = new Scrollbars(pre);
                }
            }
        }
    }
...
// ==UserScript==
// @name      ondemand scrollbar plugin - test
// @include   *
// ==/UserScript==
(function() {
